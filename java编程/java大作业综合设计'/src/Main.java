...
public class Main extends JFrame //implements  ActionListener
{
    ...
    public Main(){
        ...
        JMenuItem item4=new JMenuItem("Show all");
        JMenuItem item5=new JMenuItem("Search by age");
        bar.add(fileMenu);
        bar.add(addMenu);
        bar.add(searchMenu);
      ...
      //上面这段主要是为了判断性别是男还是女
        ...
        item5.addActionListener(//用年龄作为索引来进行查找
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        SearchByAge index=new SearchByAge();
                        remove(getContentPane());
